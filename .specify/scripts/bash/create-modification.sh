...
# Copy template
MODIFY_TEMPLATE="$REPO_ROOT/.specify/extensions/workflows/modify/modification-spec-template.md"
...
if [ -x "$IMPACT_SCANNER" ]; then
    ...
    echo "" >> "$IMPACT_FILE"
    "$IMPACT_SCANNER" "$FEATURE_DIR" "$MOD_DIR" "$MOD_DESCRIPTION" >> "$IMPACT_FILE" 2>&1 || true
else
